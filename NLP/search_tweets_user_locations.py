...
tweets = tweepy.Cursor(api.search,q="#CABPolitics -filter:retweets",count=100,
                           lang="en",
                           since="2019-12-14").items(500)

users_locs = [[tweet.user.screen_name, tweet.user.location] for tweet in tweets]
df = pd.DataFrame(users_locs,  columns=['user', "location"])
names = df.user.unique()
locs = list(df.location.unique())
df.location

df['new_loc'] = df['location'].apply(lambda x: x.split(',')[0].lower())

d = df.groupby(['new_loc']).count()
